...
from ByCap import AnticaptchaClient, RecaptchaV3TaskProxyless
...
def get_token(form_html):
    ...
    task = RecaptchaV3TaskProxyless(
        website_url=url, website_key=website_key, page_action=page_action, min_score=0.7
    )
...
def form_submit(page_action, token):
    return requests.post(
        ...
        params={"action": page_action, "token": token},
    ).json()
